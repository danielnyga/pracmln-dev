...
members : {
    ...
    main : function() {
        ...
        var ctr_pracmln = document.getElementById("mln_container",
                                                    true,
                                                    true);
        var inline_content = new qx.ui.root.Inline(ctr_pracmln,true,true);
        ...
        // ctr_scroll_infable container
        var ctr_mainctr_scroll_inf = new qx.ui.container.Scroll().set({
            width: 1024,
            height: 768
        });
        ...
        ctr_pracmln.addEventListener("resize", function() {
        ...
        document.addEventListener("roll", function(e) {
            this[0].ctr_scroll_infTop = this[0].ctr_scroll_infTop + e.delta.y;
            this[0].ctr_scroll_infLeft = this[0].ctr_scroll_infLeft + e.delta.x;
        }, this);
        ...
        // main container (contains control panel and outer splitpane)
        var ctr_inf = new qx.ui.container
                                     .Composite(new qx.ui.layout.HBox());

        // outer splitpane (contains inference settings and graph
        // visualization container)
        var ctr_mlnform_inference = new qx.ui.container
                                          .Composite(new qx.ui.layout.VBox());
        
        var ctr_mlnform_inf = this.buildMLNForm();
        
        var ctr_scroll_inf =  new qx.ui.container.Scroll().set({
            width: 750
        });

        var ctr_graphviz_inf = new qx.ui.container
                                .Composite(new qx.ui.layout.Canvas()).set(
        {
            height: .5*document.getElementById("page", true, true).offsetHeight
        });
        this._ctr_graphviz_inf = ctr_graphviz_inf;

        // visualization elements
        var grp_vizembed_inf = new qx.ui.groupbox.GroupBox("Ground Markov Random Field");
        var vizLayout = new qx.ui.layout.Grow();
        grp_vizembed_inf.setLayout(vizLayout);
        
        var html_viz_str = "<div id='viz' style='width: 100%; height: 100%;'></div>";
        var html_viz = new qx.ui.embed.Html(html_viz_str);

        var img_wait_inf = new qx.ui.basic.Image();
        img_wait_inf.setSource('/mln/static/images/wait.gif');
        ...
        var img_legend = new qx.ui.basic.Image();
        img_legend.setSource('/mln/static/images/legend.png');
        img_legend.getContentElement().setAttribute('id', 'legend');
        img_legend.setWidth(56);
        img_legend.setHeight(37);
        img_legend.setScale(1);
        this._img_legend = img_legend;

        var img_pracmlnlogo_inf = new qx.ui.basic.Image('/mln/static/images/pracmln-logo.png');
        img_pracmlnlogo_inf.setWidth(225);
        img_pracmlnlogo_inf.setHeight(69);
        img_pracmlnlogo_inf.setScale(1);
        ...
        var ctr_barchart = new qx.ui.container
                                    .Composite(new qx.ui.layout.Grow());
        ctr_barchart.getContentElement().setAttribute("id","dia");
        ctr_barchart.getContentElement().setStyle("overflow","scroll",true);
        
        var layout_grp_diaembed = new qx.ui.layout.Grow();
        var grp_diaembed = new qx.ui.groupbox.GroupBox("Inference Result");
        grp_diaembed.setLayout(layout_grp_diaembed);

        var txtarea_results_inf = new qx.ui.form.TextArea("").set({
            font: qx.bom.Font.fromString("14px monospace")
        });
        txtarea_results_inf.setReadOnly(true);
        this._txtarea_results_inf = txtarea_results_inf;

        var splitpane_inner_inf = new qx.ui.splitpane.Pane("vertical");
        splitpane_inner_inf.setHeight(.5*document.getElementById("page",
                                                                 true,
                                                                 true)
                                                  .offsetHeight);
        var splitpane_inf = new qx.ui.splitpane.Pane("vertical");
        ...
        var win_condprob = new qx.ui.window.Window("Conditional Probability");
        win_condprob.setWidth(.2 * document.getElementById("page",
                                                           true,
                                                           true).offsetWidth);
        win_condprob.setHeight(.1 * document.getElementById("page",
                                                            true,
                                                            true).offsetWidth);
        win_condprob.setShowMinimize(false);
        win_condprob.setLayout(new qx.ui.layout.Canvas());
        win_condprob.setContentPadding(4);
        win_condprob.add(img_condprob);
        win_condprob.open();
        win_condprob.hide();
        this._win_condprob = win_condprob;

        /* ************************ LISTENERS ********************************/

        // reposition graph when inference settings are shown/hidden
        ctr_graphviz_inf.addListener('resize', function(e) {
            if (typeof this._graph != 'undefined') {
                var vizSize = ctr_graphviz_inf.getInnerSize();
                var bounds = ctr_graphviz_inf.getBounds();
        ...
        win_condprob.addListener("close", function() {
            this.__chkbxShowCondProb.setValue(false);
        }, this);

        // resize image to fit in window
        win_condprob.addListener("resize", function(e) {
        ...
        img_condprob.addListener("changeSource", function(e) {
            var ratio =  typeof this._imgRatio != 'undefined'? this._imgRatio : 1;
            var newWidth = win_condprob.getInnerSize().width - 10;
            var newHeight = win_condprob.getInnerSize().height - 30;
            if (newWidth / ratio <= newHeight) {
                newHeight = newWidth / ratio;
            } else {
                newWidth = newHeight * ratio;
        ...
        ctr_scroll_inf.add(ctr_mlnform_inf)

        ctr_mlnform_inference.add(img_pracmlnlogo_inf);
        ctr_mlnform_inference.add(ctr_scroll_inf, {flex: 1});

        grp_vizembed_inf.add(html_viz);

        ctr_graphviz_inf.add(grp_vizembed_inf, {width: "100%", height: "100%"});
        ctr_graphviz_inf.add(img_wait_inf, { left: "50%", top: "50%"});
        ctr_graphviz_inf.add(img_legend, { left: 10, top: 25});

        grp_diaembed.add(ctr_barchart);

        splitpane_inner_inf.add(grp_diaembed, 1);
        splitpane_inner_inf.add(txtarea_results_inf, 2);

        splitpane_inf.add(ctr_graphviz_inf, 0);
        splitpane_inf.add(splitpane_inner_inf, 1);

        ctr_inf.add(ctr_mlnform_inference);
        ctr_inf.add(splitpane_inf, {flex: 1});

        /* ********************** LEARNING PAGE ******************************/

        /* ********************** CREATE ELEMENTS ****************************/

        var ctr_learn = new qx.ui.container
                                    .Composite(new qx.ui.layout.HBox());

        // form
        var img_pracmlnlogo_learn = img_pracmlnlogo_inf.clone();
        
        var ctr_mlnform_learning = this.buildMLNLearningForm();
        
        var ctr_scroll_learn =  new qx.ui.container.Scroll().set({
            width: 750
        });
        
        var ctr_mlnform_learn = new qx.ui.container.Composite(new qx.ui.layout.VBox());
        

        var grp_vizembed_learn = new qx.ui.groupbox.GroupBox("Learned MLN");
        var vizLayout = new qx.ui.layout.Grow();
        grp_vizembed_learn.setLayout(vizLayout);
        this._img_wait_learn = img_wait_inf.clone();
        this._img_wait_learn.getContentElement().setAttribute('id', 'waitImgLrn');
        this.__txtarea_mln_infviz = new qx.ui.form.TextArea("");
        this.__txtarea_mln_infviz.getContentElement().setAttribute("id", 'mlnResultArea');
        var ctr_graphviz_learn = new qx.ui.container.Composite(new qx.ui.layout.Canvas());
        this._ctr_graphviz_learn = ctr_graphviz_learn;

        this.__txtAResultsLrn = this._txtarea_results_inf.clone();

        var splitpane_learn = new qx.ui.splitpane.Pane("vertical");


        /* ************************ LISTENERS ********************************/

        ctr_barchart.addListener('resize', function(e) {
                if (typeof this['_barChartdia'] != 'undefined') {
                  var vizSize = ctr_barchart.getInnerSize();
        ...
        ctr_scroll_learn.add(ctr_mlnform_learning)
        
        ctr_mlnform_learn.add(img_pracmlnlogo_learn);
        ctr_mlnform_learn.add(ctr_scroll_learn, {flex: 1});
        
        grp_vizembed_learn.add(this.__txtarea_mln_infviz);
        
        ctr_graphviz_learn.add(grp_vizembed_learn, { width: "100%", height: "100%"});
        ctr_graphviz_learn.add(this._img_wait_learn, {left: "50%", top: "50%"});
        
        splitpane_learn.add(ctr_graphviz_learn, 2);
        splitpane_learn.add(this.__txtAResultsLrn, 1);
        
        ctr_learn.add(ctr_mlnform_learn);
        ctr_learn.add(splitpane_learn, {flex: 1});
        ...
        inferencePage.add(ctr_inf, {width: "100%", height: "100%"});
        ...
        learningPage.addListener("appear", function(e) {
                    ...
                    // are not yet created
                    this.highlight('tDataArea');
                    this.highlight('mlnAreaLrn');
                    this.highlight('mlnResultArea');
        }, this);
        learningPage.setLayout(new qx.ui.layout.Grow());
        learningPage.add(ctr_learn, {width: "100%", height: "100%"});
        ...
        ctr_mainctr_scroll_inf.add(ctr_main, {width: "100%", height: "100%"});
        inline_content.add(ctr_mainctr_scroll_inf, {width: "100%", height: "100%"});
        this.getRoot().add(win_condprob, {left:20, top:20});
        this._init();
    ...
    */
    buildMLNForm : function() {
        ...
        this.__txtarea_mln_inf = new qx.ui.form.TextArea("");
        this.__txtarea_mln_inf.getContentElement().setAttribute("id", 'mlnArea');
        this.__mlnAreaContainer.add(this.__txtarea_mln_inf);
        ...
        // listeners
        this.__slctEMLN.addListener("changeSelection", this.update_text_emln, this);
        ...
        this.__slctMLN.addListener("changeSelection", this.update_text_mln_inf, this);
        this.__slctProjectInf.addListener("changeSelection", this._change_example_inf ,this);
        this.__slctEvidence.addListener("changeSelection", this.update_text_evidence, this);
        ...
        this.__chkbxShowCondProb.addListener('changeValue', function(e) {
                e.getData() ? this._win_condprob.show() : this._win_condprob.hide();
            }, this);
    ...
    */
    buildMLNLearningForm : function() {
        ...
        this.__txtarea_mln_learn = new qx.ui.form.TextArea("");
        this.__txtarea_mln_learn.getContentElement().setAttribute("id", 'mlnAreaLrn');
        this.__containerMLNAreaLrn.add(this.__txtarea_mln_learn);
        ...
        this.__slctMLNLrn.addListener("changeSelection", this.update_text_mln_learn, this);
        this.__slctTDataLrn.addListener("changeSelection", this.update_text_tdata, this);
    ...
    _get_inf_status : function(timeout) {
        ...
        req.addListener("success", function(e) {
            ...
            if (response.status == true) {
                ...
                this._txtarea_results_inf.setValue(response.output);
                this._txtarea_results_inf.getContentElement().scrollToY(10000);
                ...
                if (response.img !== '') {
                  this._img_condprob.setSource('data:image/png;base64,' + response.condprob.png);
                }
            } else {
                this._get_inf_status(timeout);
            }
    ...
    */
    _start_learning : function(e) {
            this.__txtarea_mln_infviz.setValue('');
            this.highlight('mlnResultArea');
            this.show_wait_animation("learn", true);
            var that = this;
            this.loadGraph();
    ...
    */
    _get_lrn_status : function(timeout) {
        ...
        req.addListener("success", function(e) {
            ...
            if (response.status == true) {
                this.show_wait_animation("learn", false);
                this.__txtarea_mln_infviz.setValue(response.learnedmln);
                this.highlight('mlnResultArea');
    ...
    */
    _set_config : function (isinfpage, config, mlnfiles, dbfiles, methods) {
        if (isinfpage) {
            ...
            this.__txtarea_mln_inf.setValue('');
            this.highlight(this.__txtarea_mln_inf.getContentElement().getAttribute('id'));
            ...
            this.highlight(this.__txtAEvidence.getContentElement().getAttribute('id'));
            ...
            this.__txtarea_mln_learn.setValue('');
            this.highlight(this.__txtarea_mln_learn.getContentElement().getAttribute('id'));
            ...
            this.highlight(this.__txtATDataLrn.getContentElement().getAttribute('id'));
    ...
    */
    calculate_redrawing : function(oldRes, newRes) {
      var toBeRemoved = [];
      var toBeAdded = [];
      var remove;
      var add;
    ...
    */
    update_text_mln_inf : function(e) {
        if (e.getData().length > 0) {
            var selection = e.getData()[0].getLabel();
            this.update_text(selection, this.__txtarea_mln_inf);
        } else {
            this.__txtarea_mln_inf.setValue('');
        }
    },
    ...
    */
    update_text_emln : function(e) {
        if (e.getData().length > 0) {
            var selection = e.getData()[0].getLabel();
            this.update_text(selection, this.__txtAEMLN);
        } else {
            this.__txtAEMLN.setValue('');
        }
    },
    ...
    */
    update_text_evidence : function(e) {
        if (e.getData().length > 0) {
             var selection = e.getData()[0].getLabel();
             this.update_text(selection, this.__txtAEvidence);
        } else {
            this.__txtAEvidence.setValue('');
        }
    },
    ...
    */
    update_text_mln_learn : function(e) {
        if (e.getData().length > 0) {
            var selection = e.getData()[0].getLabel();
            this.update_text(selection, this.__txtarea_mln_learn);
        } else {
            this.__txtarea_mln_learn.setValue('');
        }
    },
    ...
    */
    update_text_tdata : function(e) {
        if (e.getData().length > 0) {
            var selection = e.getData()[0].getLabel();
            this.update_text(selection, this.__txtATDataLrn);
        } else {
            this.__txtATDataLrn.setValue('');
        }
    },
    ...
    */
    update_text : function(selection, area) {
        var that = this;
        var project = (this.__tabview.isSelected(this.__inferencePage) ? this.__slctProjectInf : this.__slctProjectLrn).getSelection()[0].getLabel();
        var req = new qx.io.request.Xhr();
        ...
        req.addListener("success", function(e) {
            ...
            this.highlight(area.getContentElement().getAttribute('id'));
            return;
        }, this);
        req.send();
    },
